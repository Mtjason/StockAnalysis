...
def parse_broker_info(input_text):
    # 第一種匹配規則
    match1 = re.search(r"GenLink2stk\('(.+?)','(.+?)'\);", input_text)
    if match1:
        raw_broker_id, broker_name = match1.groups()
        broker_id = re.sub(r'\D', '', raw_broker_id)  # 移除非數字字符
        return broker_name, broker_id

    # 第二種匹配規則
    match2 = re.search(r"Link2Stk\('(.+?)'\);.*?>(.+?)<", input_text, re.DOTALL)
    if match2:
        broker_id, raw_broker_name = match2.groups()
        broker_name = re.sub(r'^[a-zA-Z0-9]+', '', raw_broker_name).strip()  # 移除開頭數字並去除多餘空格
        return  broker_name.strip(), broker_id
...
for segment in segments:
    # 以 '!' 分割每一組資料
    group_items = segment.split('!')
    group_id, group_name = group_items[0].split(',')
    
    # 處理每一組子項目
    for item in group_items[1:]:
        subgroup_id, subgroup_name = item.split(',')
        data.append([group_id, group_name, subgroup_id, subgroup_name])

# 轉換成 DataFrame
#df_brokers = pd.DataFrame(data, columns=["GroupID", "Group name", "SubgroupID", "Subgroup name"])
...
sql = """
INSERT IGNORE INTO twstockinfo.twstockbroker (brokerSubID, brokerName, brokerID, datetime, id, name, buydollar, selldollar, offset)
...
keepGoing = functions.stock_days(2021, 12)
stock_openday =  keepGoing

broker_data = []

for i in range(267,268): #len(stock_openday)
#for i in range(490, len(stock_openday)): #len(stock_openday)
    date = stock_openday[i].date
    print('Processing ' + str(date))
    for brokerID in data :
        # 定義查詢參數
        params = {
            "a": brokerID[0], #"6010"
            "b":  brokerID[2], #0036003000310064"
            "c": "B", #'B' 金額 ; 'E' 張數
            "e": functions.date_font(date,'yuanta'), #"2024-12-4"
            "f": functions.date_font(date,'yuanta'), #"2024-12-4"
        }
        ...
        # Extract "買超" table
        buy_table = soup.find('td', string='買超').find_parent('table')
        buy_data = []
        for row in buy_table.find_all('tr')[2:]:  # Skip the header rows
            cells = row.find_all('td')
            if len(cells) == 4:  # Ensure it is a valid data row
                stock_name, stock_id = parse_broker_info(str(cells[0]))
                buy_data.append({
                    "brokerID" : brokerID[0],
                    "brokerSubID" : brokerID[2],
                    "brokerName" : brokerID[3],                    
                    "id": stock_id,
                    "name": stock_name,
                    'datetime': date,
                    "buydollar": cells[1].get_text(strip=True).replace(',', ''),
                    "selldollar": cells[2].get_text(strip=True).replace(',', ''),
                    "offset": cells[3].get_text(strip=True).replace(',', '')
                })
        
        # Extract "賣超" table
        sell_table = soup.find('td', string='賣超').find_parent('table')
        sell_data = []
        for row in sell_table.find_all('tr')[2:]:  # Skip the header rows
            cells = row.find_all('td')
            if len(cells) == 4:  # Ensure it is a valid data row
                stock_name, stock_id = parse_broker_info(str(cells[0]))
                sell_data.append({
        ...
        broker_data = broker_data + buy_data + sell_data
        if len(broker_data) >= 1000 :
            # 將字典轉換為元組並確保數值型態
            broker_data_tuples = [
                (
                    item['brokerSubID'],
            ...
            cursor.executemany(sql, broker_data_tuples)
            connection.commit()
            broker_data = []
    if broker_data :
        cursor.executemany(sql, broker_data_tuples)
